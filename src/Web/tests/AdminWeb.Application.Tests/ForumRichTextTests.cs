...
/// La validación es la misma que en el escritorio, pero lo que evita cambió con la web: allí un
/// enlace acababa en Process.Start y un esquema sin filtrar convertía una publicación en un
/// lanzador de programas; aquí acaba en el href de un ancla, y un javascript: o un data:text/html
/// se ejecutan en el navegador de quien lee, con su sesión. Por eso hay tantas pruebas de lo que NO
/// debe enlazarse como de lo que sí.
/// </summary>
public class ForumRichTextTests
{
    ...
    public void ElTextoVisible_NoCambia_CuandoLaDireccionVaSuelta()
    {
        ...
        // Analizar añadiera o quitara un carácter, el enlace caería sobre las letras de al lado.
    ...
    public void UnEsquemaQueNoEsWeb_NoSeVuelvePulsable(string peligrosa)
    {
        ...
        // enlazara, pulsar en una publicación cualquiera ejecutaría lo que hubiera detrás.
    ...
    // ── El punto de extensión del saneado ────────────────────────────────────────

    /// <summary>
    /// Fija por escrito lo que HOY hace Sanear: nada. No es un descuido, es el estado conocido —
    /// mientras devuelva el texto tal cual, la protección real es que el cuerpo se pinte como TEXTO
    /// y no como marcado. Cuando entre la librería de saneado, esta prueba será la primera en
    /// fallar, que es exactamente el aviso que se busca.
    /// </summary>
    [Fact]
    public void Sanear_HoyEsLaIdentidad_YEsoEstaDocumentado()
    {
        const string peligroso = "<script>alert(1)</script>";
        Assert.Equal(peligroso, ForumRichText.Sanear(peligroso));
        Assert.Equal("", ForumRichText.Sanear(null));
    }
}
