...
{
    private static AttendanceService Svc(AppDbContext db, ICurrentUser cu) =>
        new(db, cu, new AuditService(db, cu, new OrigenDePrueba()), new OrigenDePrueba());

    private static ICurrentUser Usuario(AppDbContext db, int userId, string nombre,
    ...
    [Fact]
    public async Task MarcarEntrada_AbreElRegistroConHoraUtcYOrigen()
    {
        var db = TestDb.New();
        var (ok, _) = await Svc(db, Usuario(db, 1, "Ana")).MarcarEntradaAsync();
    ...
    [Fact]
    public async Task LasMarcasSeGuardanEnUtc()
    {
        var db = TestDb.New();
        var svc = Svc(db, Usuario(db, 1, "Ana"));
        await svc.MarcarEntradaAsync();
        await svc.MarcarSalidaAsync();
    ...
    [Fact]
    public async Task MarcarEntrada_ConEntradaAbiertaHoy_SeRechaza()
    {
        var db = TestDb.New();
        var svc = Svc(db, Usuario(db, 1, "Ana"));
        await svc.MarcarEntradaAsync();

        var (ok, mensaje) = await svc.MarcarEntradaAsync();
    ...
    [Fact]
    public async Task MarcarEntrada_ConSalidaYaMarcadaHoy_SeRechaza()
    {
        var db = TestDb.New();
        var svc = Svc(db, Usuario(db, 1, "Ana"));
        await svc.MarcarEntradaAsync();
    ...
    [Fact]
    public async Task MarcarSalida_CierraElRegistroComoManual()
    {
        var db = TestDb.New();
        var svc = Svc(db, Usuario(db, 1, "Ana"));
        await svc.MarcarEntradaAsync();

        var (ok, _) = await svc.MarcarSalidaAsync();
    ...
    [Fact]
    public async Task MarcarSalida_SinEntradaAbierta_SeRechaza()
    {
        var db = TestDb.New();

        var (ok, mensaje) = await Svc(db, Usuario(db, 1, "Ana")).MarcarSalidaAsync();
    ...
    [Fact]
    public async Task MarcarSalida_DosVeces_LaSegundaSeRechaza()
    {
        var db = TestDb.New();
        var svc = Svc(db, Usuario(db, 1, "Ana"));
        await svc.MarcarEntradaAsync();
    ...
    [Fact]
    public async Task Olvido_SeCierraConLaUltimaSenalAutomaticaDeEseDia_NoConLaHoraDeAhora()
    {
        var db = TestDb.New();
        ...
        // arranque justo después de medianoche coloca la entrada y la telemetría en días distintos y
        // la prueba falla sin que nada esté mal: el defecto estaba en la prueba, no en el servicio.
        ...
        await svc.MarcarEntradaAsync();   // la de hoy dispara el cierre del olvido
    ...
    [Fact]
    public async Task Olvido_SinTelemetria_SeCierraConDuracionCero_YQuedaMarcado()
    {
        var db = TestDb.New();
        var svc = Svc(db, Usuario(db, 1, "Ana"));
    ...
    [Fact]
    public async Task MarcarSalida_ConEntradaDeOtroDia_LaCierraComoOlvidoYNoLaCobraHoy()
    {
        var db = TestDb.New();
        var svc = Svc(db, Usuario(db, 1, "Ana"));
        ...
        var (ok, mensaje) = await svc.MarcarSalidaAsync();
    ...
    [Fact]
    public async Task TurnoQueCruzaLaMedianoche_SePuedeCerrarNormalmente()
    {
        var db = TestDb.New();
        var svc = Svc(db, Usuario(db, 1, "Ana"));
    ...
    [Fact]
    public async Task Olvido_ConSenalAnteriorALaEntrada_NoDaDuracionNegativa()
    {
        var db = TestDb.New();
        var svc = Svc(db, Usuario(db, 1, "Ana"));
    ...
    [Fact]
    public async Task DosRegistrosConsecutivosQueSoloSeTocan_SeAceptan()
    {
        var db = TestDb.New();
        Usuario(db, 1, "Ana");
        ...
        Assert.True((await svc.CrearRegistroManualAsync(1, dia.AddHours(8), dia.AddHours(14), "mañana")).ok);

        // El segundo empieza justo donde acabó el primero: no se encima con nada.
        var (ok, mensaje) = await svc.CrearRegistroManualAsync(1, dia.AddHours(14), dia.AddHours(18), "tarde");

        Assert.True(ok, mensaje);
    }

    [Fact]
    public async Task CrearRegistroManual_SobreUnaJornadaAbierta_SeRechaza()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var jefe = Usuario(db, 9, "Jefe", UserRole.Admin);
        await Svc(db, ana).MarcarEntradaAsync();   // queda abierta ahora mismo
        ...
        var (ok, mensaje) = await Svc(db, jefe)
            .CrearRegistroManualAsync(1, DateTime.Now.AddMinutes(-30), null, "alta encimada");
    ...
    [Fact]
    public async Task MisRegistros_SoloDevuelveLosPropios()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var beto = Usuario(db, 2, "Beto");
        await Svc(db, ana).MarcarEntradaAsync();
        await Svc(db, beto).MarcarEntradaAsync();

        var mios = await Svc(db, ana).MisRegistrosAsync(DateTime.Today, DateTime.Today);
    ...
    [Fact]
    public async Task SinSesion_MarcarLanzaAuthorizationException()
    {
        var db = TestDb.New();
        var svc = Svc(db, UsuarioDePrueba.Anonimo());

        await Assert.ThrowsAsync<AuthorizationException>(() => svc.MarcarEntradaAsync());
        await Assert.ThrowsAsync<AuthorizationException>(() => svc.MarcarSalidaAsync());
    }

    [Fact]
    public async Task SolicitarCorreccion_SobreRegistroAjeno_SeRechaza()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var beto = Usuario(db, 2, "Beto");
        ...
        await Assert.ThrowsAsync<AuthorizationException>(() =>
            Svc(db, beto).SolicitarCorreccionAsync(idDeAna, "no era mi horario"));
    }

    [Fact]
    public async Task SolicitarCorreccion_ExigeMotivo_YLoGuarda()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var svc = Svc(db, ana);
        ...
        Assert.False((await svc.SolicitarCorreccionAsync(id, "   ")).ok);

        var (ok, _) = await svc.SolicitarCorreccionAsync(id, "entré a las 8, no a las 9");
        Assert.True(ok);
        var r = db.AttendanceRecords.AsNoTracking().Single();
    ...
    [Fact]
    public async Task AsistenciaDelDia_RequiereAdmin()
    {
        var db = TestDb.New();
        var svc = Svc(db, Usuario(db, 1, "Ana"));

        await Assert.ThrowsAsync<AuthorizationException>(() => svc.AsistenciaDelDiaAsync(DateTime.Today));
    }

    [Fact]
    public async Task AsistenciaDelDia_CruzaOficialConTelemetria_YCalculaLosDeltas()
    {
        var db = TestDb.New();
        Usuario(db, 1, "Ana");
        var jefe = Usuario(db, 9, "Jefe", UserRole.Admin);
        ...
        var dia = DateTime.Today.AddDays(-1);
        Assert.True((await Svc(db, jefe).CrearRegistroManualAsync(1, dia.AddHours(9), dia.AddHours(18), "alta de prueba")).ok);
        ...
        var fila = (await Svc(db, jefe).AsistenciaDelDiaAsync(dia)).Single(f => f.UserId == 1);
    ...
    [Fact]
    public async Task AsistenciaDelDia_SenalaAQuienTuvoTelemetriaPeroNoMarco()
    {
        var db = TestDb.New();
        Usuario(db, 1, "Ana");
        var jefe = Usuario(db, 9, "Jefe", UserRole.Admin);
    ...
    [Fact]
    public async Task CorregirRegistro_RequiereAdminYExigeMotivo()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var jefe = Usuario(db, 9, "Jefe", UserRole.Admin);
        ...
        await Assert.ThrowsAsync<AuthorizationException>(() =>
            Svc(db, ana).CorregirRegistroAsync(id, DateTime.Now.AddHours(-3), DateTime.Now.AddHours(-1), "porque sí"));

        Assert.False((await Svc(db, jefe).CorregirRegistroAsync(id, DateTime.Now.AddHours(-3), null, "  ")).ok);
    }

    [Fact]
    public async Task CorregirRegistro_SalidaAntesDeEntrada_SeRechaza()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var jefe = Usuario(db, 9, "Jefe", UserRole.Admin);
        ...
        var (ok, mensaje) = await Svc(db, jefe)
            .CorregirRegistroAsync(id, DateTime.Now.AddHours(-1), DateTime.Now.AddHours(-3), "dedazo");
    ...
    [Fact]
    public async Task CorregirRegistro_EnElFuturo_SeRechaza()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var jefe = Usuario(db, 9, "Jefe", UserRole.Admin);
        ...
        var (ok, mensaje) = await Svc(db, jefe)
            .CorregirRegistroAsync(id, DateTime.Now.AddHours(2), DateTime.Now.AddHours(4), "adelanto");
    ...
    [Fact]
    public async Task CorregirRegistro_GuardaRastroEnLaFila_YViejoYNuevoEnLaBitacora()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var jefe = Usuario(db, 9, "Jefe", UserRole.Admin);
        var svcAna = Svc(db, ana);
        await svcAna.MarcarEntradaAsync();
        int id = db.AttendanceRecords.AsNoTracking().Single().Id;
        await svcAna.SolicitarCorreccionAsync(id, "entré antes");

        var entrada = DateTime.Now.AddHours(-8);
        var salida  = DateTime.Now.AddHours(-1);
        var (ok, _) = await Svc(db, jefe).CorregirRegistroAsync(id, entrada, salida, "llegó a las 8, lo confirmé");
    ...
    [Fact]
    public async Task CrearRegistroManual_RequiereAdmin_YQuedaConRastro()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var jefe = Usuario(db, 9, "Jefe", UserRole.Admin);
        ...
        await Assert.ThrowsAsync<AuthorizationException>(() =>
            Svc(db, ana).CrearRegistroManualAsync(1, DateTime.Now.AddHours(-5), DateTime.Now.AddHours(-1), "trabajó sin la app"));

        var (ok, _) = await Svc(db, jefe)
            .CrearRegistroManualAsync(1, DateTime.Now.AddHours(-5), DateTime.Now.AddHours(-1), "trabajó sin la app");
    ...
    [Fact]
    public async Task CrearRegistroManual_QueSeEncimaConOtro_SeRechaza()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var jefe = Usuario(db, 9, "Jefe", UserRole.Admin);
        var svcJefe = Svc(db, jefe);
        await svcJefe.CrearRegistroManualAsync(1, DateTime.Now.AddHours(-5), DateTime.Now.AddHours(-1), "primero");

        var (ok, mensaje) = await svcJefe.CrearRegistroManualAsync(1, DateTime.Now.AddHours(-3), DateTime.Now.AddHours(-2), "encimado");
    ...
    [Fact]
    public async Task CorreccionesPendientes_CuentaSoloLasQueEsperanRespuesta()
    {
        var db = TestDb.New();
        var ana = Usuario(db, 1, "Ana");
        var jefe = Usuario(db, 9, "Jefe", UserRole.Admin);
        ...
        Assert.Equal(0, await Svc(db, jefe).CorreccionesPendientesAsync());

        await svcAna.SolicitarCorreccionAsync(id, "revisen mi hora");
        Assert.Equal(1, await Svc(db, jefe).CorreccionesPendientesAsync());

        await Svc(db, jefe).CorregirRegistroAsync(id, DateTime.Now.AddHours(-4), null, "corregido");
        Assert.Equal(0, await Svc(db, jefe).CorreccionesPendientesAsync());
    }
}
