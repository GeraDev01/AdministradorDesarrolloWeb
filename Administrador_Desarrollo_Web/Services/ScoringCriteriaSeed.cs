...
public static class ScoringCriteriaSeed
{
    ...
    /// <see cref="Sembrar"/>: si corriera después, el sembrado ya habría insertado la versión nueva
    /// y la base acabaría con el criterio duplicado, uno con cada nombre.
    ///
    ...
    /// </summary>
    public static int MigrarNomenclatura(AppDbContext db)
    {
        // Una sola lectura: son ~90 criterios y hacer una consulta por cada uno son ~90 viajes a
        // una base que se lee por red.
        var porNombre = db.ScoringCriteria.ToDictionary(c => c.Name, StringComparer.Ordinal);
        int cambiados = 0;
    ...
    {
        var existentes = db.ScoringCriteria.Select(c => c.Name).ToHashSet(StringComparer.Ordinal);
        bool agregado = false;
